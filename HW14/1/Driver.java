...
public class Driver {
	...
	public Object generateString() {
		identifier++;
		String s = new Date().toString() + "_" + Math.random();
	...
	public void initTest() {
		...
		for (int index = 0; index < MAX_ELEMENTS; index++)
			aHashSet.add(allObjects[index] = objectGenerator()); // hashcode of
																	// MeanObject
																	// ...
	}
	...
	public void addTest() {
		...
		System.out.println("soMany: " + soMany);
		aHashSet.add("theSame");
		aHashSet.add("theSame");
		/*
		 * The reason the number of elements in the hash set is still
		 * "soMany + 1" is becoz in a hash set, if the same element is added, it
		 * doesn't occupy a new position since hash sets never add duplicates.
		 */
		System.out.println("Size after adding 'theSame' twice: " + aHashSet.size());
	...
	public void containsTest() {
		...
		for (int index = 0; index < MAX_ELEMENTS ; index++ ){
			if(allObjects[index] == null){
				System.out.println("NULL FOUND IN allObjects[]....containsTest()");
			}
		}	
		for (int index = 0; index < MAX_ELEMENTS; index++) {
		...
		passed = true;
		//lines 111 to 113 successfully execute..... but line 100 to 106 doesn't..why?
		aHashSet.remove(allObjects[0]);
		if (aHashSet.contains(allObjects[0]))
			passed = false;

		System.out
				.println("	containsTest: " + (passed ? "passed" : " failed "));
	}
	...
	public void iteratorTest() {
		initTest();
		Iterator<Object> aIterator = aHashSet.iterator();
		if (aIterator == null){
			//System.out.println("aIterator is null!");
			passed = false;
		} else {
			try {
				if (aIterator.hasNext()) {
					//System.out.println("Yes, aIterator did have a next!");
					aHashSet.remove(aIterator.next());
				} else {
					passed = false;
				}
			} catch (Exception e) {
				//System.out.println("Exception is being thrown in iteratorTest()");
				passed = false;
			}
		}
		System.out
				.println("	iteratorTest: " + (passed ? "passed" : " failed "));
	}
	...
	public void testIt(String whichOne) {
		...
		for (int index = MIN_KIND_OF_OBJECT_KIND; index <= MAX_KIND_OF_OBJECT_KIND; index++) {
			...
			testOneKind();
			System.out.println("	" + objectKind + ":time:		"
					+ (System.currentTimeMillis() - milliSeconds));
		}
	}
