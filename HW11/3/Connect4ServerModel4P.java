/**
 * Connect4ServerModel4P.java
 * 
 * @version $ID: Connect4ServerModel4P.java, v 1.8 11/7/2015 7:02pm
 * 
...
public class Connect4ServerModel4P implements Connect4FieldInterface {
	...
	public boolean checkIfPiecedCanBeDroppedIn(int column) {
		...
		int flg = 0;
		if (column >= 0) {
			for (int index = (Connect4Server4P.Field.length - 1); index >= 0; index--) {
				if (Connect4Server4P.Field[index][column] == '.') {
					flg = 1;
					break;
				}
			}
	...
	public void dropPieces(int column, char gamePiece) {
		...
		int flg = 0, index = 0;
		if (column >= 0) {
			for (index = (Connect4Server4P.Field.length - 1); index >= 0; index--) {
				if (Connect4Server4P.Field[index][column] == '.') {
					flg = 1;
					break;
				}
		...
		if (flg == 1) {
			Connect4Server4P.Field[index][column] = gamePiece;
		} else {
	...
	public void FollowPattern(int index, int index2, char[] orientation,
		...
		if (Connect4Server4P.Field[index][index2] == currentChar) {
			if (orientation[itr] == 'd') {
				if ((index + 1 < Connect4Server4P.Field.length)
						&& (index2 + 1 < Connect4Server4P.Field[0].length)) {
					FollowPattern(index + 1, index2 + 1, orientation, itr,
							currentChar);
					patternCount++;
				} else if (index == (Connect4Server4P.Field.length - 1)) {
					patternCount++;
				}
				...
				if ((index < Connect4Server4P.Field.length)
						&& (index2 < Connect4Server4P.Field[0].length)) {
	...
	public boolean didLastMoveWin() {
		...
		here: for (int index = 0; index < Connect4Server4P.Field.length; index++) {
			for (int index2 = 0; index2 < Connect4Server4P.Field[0].length; index2++) {
				itr = 0;
				orientIndex = 0;
				if ((((index + 1) < Connect4Server4P.Field.length))
						&& ((Connect4Server4P.Field[index][index2] == '+') || (Connect4Server4P.Field[index][index2] == '*'))
						|| (Connect4Server4P.Field[index][index2] == '#')
						|| (Connect4Server4P.Field[index][index2] == '@')) {
					char currentCharacter = Connect4Server4P.Field[index][index2];
					if (index2 < (Connect4Server4P.Field[0].length - 1)) {
						if (Connect4Server4P.Field[index][index2 + 1] == Connect4Server4P.Field[index][index2]) {
							orient[orientIndex++] = 'h';
						}
						if (Connect4Server4P.Field[index + 1][index2 + 1] == Connect4Server4P.Field[index][index2]) {
							orient[orientIndex++] = 'd';
						}
					}
					if (Connect4Server4P.Field[index + 1][index2] == Connect4Server4P.Field[index][index2]) {
	...
	public boolean isItaDraw() {
		...
		int flg = 0;
		here: for (int index = 0; index < Connect4Server4P.Field.length; index++) {
			for (int index2 = 0; index2 < Connect4Server4P.Field.length; index2++) {
				if (Connect4Server4P.Field[index][index2] == '.') {
					flg = 1;
					break here;
				}
			}
